...
const GraphContent = () => {
  ...
  if (loading) {
    return (
      <div className="flex h-screen items-center justify-center bg-brand-black px-4">
        <div className="text-brand-orange animate-pulse text-lg md:text-xl font-display text-center">Initializing Neural Map...</div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-brand-black min-h-screen overflow-hidden">
      ...
      <main className="ml-0 md:ml-64 h-screen pt-20 relative transition-all duration-300">
        {error && (
          <div className="absolute top-24 left-1/2 -translate-x-1/2 z-50 px-6 py-3 bg-red-500/10 border border-red-500/20 text-red-500 rounded-xl backdrop-blur-md text-xs md:text-sm">
            {error}
          </div>
        )}
        ...
        <div className="hidden md:flex absolute bottom-6 left-8 items-center gap-6 text-[10px] font-bold uppercase tracking-[0.2em] text-gray-600">
